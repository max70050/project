...
const FORBIDDEN_USERNAMES = [
    ...
    'terrorist', 'anarchist', 'extremist', 'radikal', 'ns', 'ss', 'sa', 'reich',
    ...
    'verschwörungstheoretiker', 'chemtrail', 'pirat', 'piracy', 'piracy', 'copyrights',
...
const SUSPICIOUS_USERNAME_WORDS = [
    ...
    'spasti', 'kanacken', 'judenhasser', 'opfer', 'spinner', 'hirni', 'idiot',
    'mutterficker', 'schnauze', 'verpiss', 'leckmich', 'scheisskerl',
    'penner', 'mongo', 'spasti', 'wichser', 'penisneid',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const footerImpressumLink = document.querySelector('footer a[href="#impressum"]');
if (footerImpressumLink) {
    footerImpressumLink.addEventListener('click', e => {
        e.preventDefault();
        showView('impressum');
    });
}
   ...
   document.addEventListener('click', (e) => {
    ...
    // NEU HINZUGEFÜGT: Klick auf "Bild senden" löst den versteckten file input aus
document.getElementById('btn-send-image').addEventListener('click', () => {
    chatImageUpload.click(); // Dies ist die entscheidende Zeile
    chatAttachMenu.classList.remove('visible'); // Schließt das Menü
});
...
form.onsubmit = async (e) => {
    ...
    if (!category) {
        showToast("Bitte wähle eine Kategorie aus.", true);
        return;
    }
    if (!difficulty) {
        showToast("Bitte wähle einen Schwierigkeitsgrad aus.", true);
        return;
    }
    // ==========================================================
    // ENDE DER KORREKTUR
...
// in app.js (im Auth-Abschnitt)

function openAuthModal() {
    // NEU: Setzt die Ansicht im Modal jedes Mal auf "Anmelden" zurück.
    loginView.hidden = false;
    registerView.hidden = true;

    // Zeigt das Modal an (wie bisher).
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Logik für Deep Linking und Routing beim Laden der Seite
  const handleInitialLoadRouting = () => {
      const hash = window.location.hash;

      // Prüfen, ob der Link eine Detail-Ansicht aufrufen soll
      if (hash.startsWith('#detail=')) {
          const adId = hash.substring(8); // Extrahiert die ID nach '#detail='
          if (adId) {
              renderDetail(adId);
          } else {
              showView("home"); // Fallback, falls die ID fehlt
          }
      } else if (hash.startsWith('#profile=')) {
          const userId = hash.substring(9);
          if (userId) {
              renderPublicProfileView(userId);
          } else {
              showView("home");
          }
      }
      else {
          // Standardverhalten: Zeige die im Hash angegebene Ansicht oder die Startseite
          const viewName = hash.replace('#', '') || 'home';
          showView(viewName);
      }
  };

  handleInitialLoadRouting();
  
  // Event-Listener für zukünftige Hash-Änderungen (z.B. bei Nutzung des Browser-Zurück-Buttons)
  window.addEventListener('hashchange', handleInitialLoadRouting);
});
...
async function renderDetail(adId) {
    // KORREKTUR START: Anzeige direkt laden, falls sie nicht im Cache ist
    let ad = ads.find(a => a.id === adId);

    // Wenn die Anzeige nicht im lokalen Array gefunden wird (z.B. bei einem Deep Link)
    if (!ad) {
        try {
            console.log(`Anzeige ${adId} nicht im Cache gefunden, lade direkt...`);
            const adDoc = await getDoc(doc(db, "ads", adId));
            if (adDoc.exists()) {
                ad = { id: adDoc.id, ...adDoc.data() };
            }
        } catch (error) {
            console.error("Fehler beim direkten Laden der Anzeige:", error);
            showToast("Anzeige konnte nicht geladen werden.", true);
            showView(lastListView || 'home');
            return;
        }
    }
 ...
 */
// app.js (Neuer Code ohne Bestätigung)

async function handleDeleteChat(chatId) {
    // Die Bestätigungsabfrage wurde entfernt. Der Code wird nun direkt ausgeführt.
    const currentUser = auth.currentUser;
    if (!currentUser) return;
    const chatDocRef = doc(db, "chats", chatId);
    try {
