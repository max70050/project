...
const showPage = (pageToShow) => {
    [welcomeSection, orderSection, profileSection, userOrdersSection, managementOrdersSection, adminAreaSection, adminToolsSection].forEach(page => {
        const displayStyle = page.id === 'order-section' ? 'grid' : (page.classList.contains('orders-display-section') ? 'flex' : 'flex');
...
const showAdminToolsPage = () => showPage(adminToolsSection);
...
const populateMinutes = (isFourteen = false) => {
    ...
    const minutes = isFourteen ? ['00'] : ['00', '15', '30', '45'];
...
managementOrdersListContainer.addEventListener('click', async (e) => {
    const target = e.target.closest('.finish-order-btn');
    if (!target) return;
    ...
    try {
        ...
        if (updatedDoc.exists() && updatedDoc.data().userAcknowledged === true) {
            await deleteDoc(orderRef);
        }
    } catch (error) {
        console.error("Fehler bei Admin-Aktion: ", error);
